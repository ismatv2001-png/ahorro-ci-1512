...
class AccountPool:
    ...
    def pick(self) -> Optional[str]:
        """Cuenta con presupuesto restante y hueco de concurrencia, round-robin
        ponderado: las que más presupuesto restante tienen salen antes."""
    ...
    def release(self, label: str, *, ok: bool = True, status: int = 200,
        ...
        """Registra el resultado del intento con esa cuenta."""
        ...
        if not ok:
            a["errors"] += 1
            if status == 402:
                a["no_balance"] = True  # marcada sin saldo: fuera de rotación
            return
